...
public class Main {
    ...
    for (int i=0 ; i < input.length && (flagTriangle || flagCircle) ; i++){
        if( input[i] instanceof Triangle ){
            TrianglePerimeter = input[i].getPerimeter();
            flagTriangle = false;
        }
        if( input[i] instanceof Circle ){
            CirclePerimeter = input[i].getPerimeter();
            flagCircle = false;
        }
    }
    ...
    for(Shape test : input){
        if( test instanceof Triangle ){
           if(test.getPerimeter() < TrianglePerimeter){
               TrianglePerimeter = test.getPerimeter();
           }
        }
        if( test instanceof Circle ){
            if(test.getPerimeter() > CirclePerimeter ) {
                CirclePerimeter = test.getPerimeter();
            }
        }
    }
    ...
    for(int i = 0 ; i< input.length ; i++){
        ...
        if( input[i] instanceof Triangle ){
            if(input[i].getPerimeter() == TrianglePerimeter){
                input[i] = null;
                nullCounter++;
            }
        }
        if( input[i] instanceof Circle ) {
            if (input[i].getPerimeter() == CirclePerimeter) {
                input[i] = null;
                nullCounter++;
            }
        }
   ...
   Shape[] output = new Shape[input.length - nullCounter];
    ...
    public static void main(String[] args) {
                String fileName = "C:\\Users\\hamza\\Downloads\\Telegram Desktop\\WS2\\WS2\\src\\shapes\\shapes.txt";
